...
namespace Repositorios.Servicios
{
    public class TipoMaterialServicio : ITipoMaterialServicio
    {
        ...
        public async Task<Response<List<TipoMaterialDTO>>> ObtenerTiposDeMaterial(long EmpresaId)
        {
            try
            {
                var ExisteMaterial = await baseDeDatos.TipoMateriales.Where(s => s.EmpresaId == EmpresaId).ToListAsync();
                if (ExisteMaterial == null || ExisteMaterial.Count == 0)
                {
                    return new Response<List<TipoMaterialDTO>>()
                    {
                        Objeto = null,
                        Mensaje = "No se encontraron tipos de material.",
                        Estado = true
                    };
                }
            ...
            catch (Exception ex)
            {
                ...
                Response<List<TipoMaterialDTO>> response = new Response<List<TipoMaterialDTO>>(){ Estado = false };
                return response;
            }
        }
